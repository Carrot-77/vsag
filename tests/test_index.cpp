...
void
TestIndex::TestBatchCalcDistanceById(const IndexPtr& index, const TestDatasetPtr& dataset, float error) {
    auto queries = dataset->query_;
    auto query_count = queries->GetNumElements();
    auto dim = queries->GetDim();
    auto gts = dataset->ground_truth_;
    ...
    for (auto i = 0; i < query_count; ++i) {
        ...
        query->NumElements(1)
            ...
            ->Float32Vectors(queries->GetFloat32Vectors() + i * dim)
            ->Owner(false);
        auto result = index->CalcBatchDistanceById(query->GetFloat32Vectors(), gts->GetIds() + (i * gt_topK), gt_topK);
        REQUIRE(result.has_value());
        for (auto j = 0; j < gt_topK; ++j) {
            REQUIRE(std::abs(gts->GetDistances()[i * gt_topK + j] - result.value()->GetDistances()[j]) < error);
        }
    }
}
